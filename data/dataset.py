...
class CoupletDataset(Dataset):
    ...
    def __len__(self):#定义如何按索引取一个样本
        return len(self.pairs)
    def __getitem__(self, idx):#写obj[index]时实际调用obj.__getitem__(index)
...
class CoupletDataModule:
    def __init__(self,config):
        ...
        def setup(self):
            ...
            print(f"Total pairs:{len(pairs)} Train:{len(self.train_pairs)}Valid:{len(self.valid_pairs)} Vocab:{self.vocab.length()}")
            
            def _extract_pair(self,item):
                if "up" in item and "down" in item: return item["up"],item["down"]
                if "input" in item and "output" in item: return item["input"],item["output"]
                if "source" in item and "target" in item: return item["source"],item["target"]
                raise KeyError(f"No fields in {list(item.keys())}")
            
            def train_dataloader(self):
                return DataLoader(CoupletDataset(self.train_pairs,self.vocab,self.config.max_len),
                                batch_size=self.config.batch_size,shuffle=True)
            def valid_dataloader(self):
                return DataLoader(CoupletDataset(self.valid_pairs,self.vocab,self.config.max_len),
